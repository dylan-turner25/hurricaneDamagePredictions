...
# loading  FEMA data -------------------------------
    ...
    # load historical ihp claims data
      ihpClaims <-read_csv("Data/IndividualsAndHouseholdsProgramValidRegistrations.csv")
      #ihpClaims <- ihpClaims[sample(nrow(ihpClaims), 10000, replace = F),] # subsample to make data easier to work with during initial coding
...
# clean ihp data -------------------------------------------------------------------  
        ...
        # filter data
        ihpClaims %>% filter(ihpClaims$ownRent == "Owner") -> ihpClaims
        ihpClaims %>% filter(ihpClaims$residenceType %in% c("House/Duplex")) -> ihpClaims
        ihpClaims %>% filter(ihpClaims$incidentType %in% c("Hurricane")) -> ihpClaims
        

        # fips code
        ihpClaims$fips <- substr(ihpClaims$censusBlockId,1,5)
        
        # add disaster declaration incident dates using fips and disasterNumber
        ihpClaims <- left_join(ihpClaims,disasterDeclarations, by = c("fips","disasterNumber"))
        
        # create a storm ID field to merge with hurricane data
        ihpClaims$storm_id <- paste0(str_to_title(tolower(trimws(str_replace(ihpClaims$declarationTitle, "HURRICANE", "")))),"-",ihpClaims$fyDeclared)
        

        # add attributes of the hurricane (windspeed, rain, etc...) from hurricane exposure data
        ihpClaims <- left_join(ihpClaims, hurricanes, by = c("storm_id","fips"))
        
        
        # data source
        ihpClaims$source <- "ihp"
        
        # state
        ihpClaims$state <- ihpClaims$damagedStateAbbreviation
        
        #indicator for flood insurance
        ihpClaims$flood_insurance <- ihpClaims$floodInsurance
        
        # year 
        ihpClaims$year <- ihpClaims$fyDeclared
      
        # damage
            # flood damage to structure/contents
            ihpClaims$flood_structure_claim <- NA 
            ihpClaims$flood_contents_claim <- NA
            
            # coverage on structure/contents
            ihpClaims$flood_structure_coverage <- NA
            ihpClaims$flood_contents_coverage <- NA
            
            # sum to get total flood damage
            ihpClaims$total_flood_damage <- NA
            
            # the amount of damage may be censored from above (ex. true damage may be $100,000 but if the policy only had
            # $50,000 of coverage than the paid claim will not reflect actual damage)
            ihpClaims$total_flood_damage_censored <- NA 
        ...
        # freeboard        
            ihpClaims$freeboard_ft <- NA
        # base flood elevation 
            ihpClaims$base_flood_elv_ft <- NA
        # basement
            ihpClaims$basement <- NA
            ihpClaims$crawlspace <- NA
        # elevated building
            ihpClaims$elevated_building <- NA
        # flood zone    
            ihpClaims$floodZone <- NA
            ihpClaims$a_zone <- NA
            ihpClaims$v_zone <- NA
            ihpClaims$x_zone <- NA
            ihpClaims$sfha <- NA
        # floors
            ihpClaims$floors <- NA
        # post firm
            ihpClaims$post_firm <- NA
        # primary_res    
            ihpClaims$primary_res <- NA
        # hurricane category
            ihpClaims$hurr_cat <- NA
        # income
            ihpClaims$hhinc <- ihpClaims$grossIncome
        

        # subset into a data frame with common variables
        ihp <- ihpClaims[,c("source","state","fips","storm_dist",
                            "tot_precip","vmax_sust","vmax_gust",
                            "sust_dur","storm_id","start_date",
                            "end_date","flood_insurance","year",
                            "flood_structure_claim","flood_structure_coverage",
...
# Combine NFIP and IHP data into one data frame ----------------------------------------------
      claims <- rbind.data.frame(nfip,ihp)
        
      # filter claims
        claims %>% drop_na(year) -> claims
        
        
      
# add inflation index ---------------------------------------------------------    
        
        # get an inflation index to adjust claims to 2020 dollars
        ...
        # add inflation index to claims data 
        claims <- left_join(claims, inflation_index, by = "year")   
...
# Add in zillow home value data ---------------------------------------
          ...
          # join zillow average county home values 
              claims <-  left_join(claims, zhvi, by = c("fips","year"))
          
          # redefine damages as a percentage of home structure value (using county average as home value)
            claims$flood_structure_claim_posv <- claims$flood_structure_claim / claims$avg_county_home_value
...
# Create indicators for if observation if from a coastal state and coastal county --------------------------
    ...
    ## Coastal State --------------------------------------------------------------      
          ...
          # add an indicator in nfip claims for if state is coastal
          claims$coastal_state <- as.numeric(claims$state %in% states_on_coast$abb)
    ...
    ## Coastal County ----------------------------------------------------------      
        # load csv file of coastal counties 
          ...
          # join with NFIP claims to add coastal_region
          claims <- left_join(claims, coastal_counties, by = c("fips"))
          
          # add indicator: = 1 if coastal county
          claims$coastal_county <- as.numeric(claims$fips %in% coastal_counties$fips)   
          
# subset data to only contain one target variable and predictor variables ----
          

# save cleaned data as a csv file --------------------------------------------
          write_csv(claims, "./Data/cleaned_data.csv")
